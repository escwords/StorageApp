...
class CreateProfileFragment : Fragment(), ImageDialogFragment.OnPhotoSelectedListener {
    ...
    private fun setUpUi(view: View, user: User) {
        ...
        loadImageWithCoil(user.photoUrl!!.toUri())

        view.findViewById<ImageView>(R.id.select_img).setOnClickListener {
            Toast.makeText(activity, "button Clicked", Toast.LENGTH_SHORT).show()
            loadImageOptionDialog()
        }
    }

    private fun loadImageOptionDialog() {
        if (readPermissionAccessApproved()) {
            navigateToDialogScreen()
        } else {
    ...
    private fun loadImageWithCoil(uri: Uri?) {
        profileImageThumbnail.load(uri) {
            crossfade(true)
            placeholder(R.drawable.jetbrains)
            transformations(CircleCropTransformation())
        }
    }

    private fun navigateToDialogScreen() {
        //requireActivity() was not giving the real activity for navigating
        val transaction = (activity as AccountActivity).supportFragmentManager.beginTransaction()
        val imageOptionDialog = ImageDialogFragment()
        imageOptionDialog.show(transaction, "DialogScreen")
